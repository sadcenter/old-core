...
/**
 * @author sadcenter on 26.10.2020
 * @project LASTCORE
 */
...
public class GuildLeaderCommand extends Command {
    ...
    public void onCommand(CommandSender executor, CommandBuilder commandBuilder, String... args) throws CommandException {
        ...
        if (g.getLeader().equalsIgnoreCase(name)) {
            ChatHelper.sendMessage(player, "&cTen gracz jest juz zastepca!");
            return;
        }
        g.setLeader(name);
        ChatHelper.sendMessage(Bukkit.getOnlinePlayers(), "&7Zastepca gildii [&3" + g.getTag() + "&7] zostal &3" + name);
    }
}
